...
namespace NetworkSimulator
{
    ...
    public static class OFJQN
    {
        public static NetworkModel CreateNetworkModel(Description description, Random random)
        {
            ...
            var Nodes = new Node[description.B.Length + description.F.Length +
                 description.J.Length + 1];
            //Источник требований
            ...
            ServiceNode[] serviceNodes = new ServiceNode[description.S.Length];
            for (int i = 0; i < description.S.Length; i++)
            {
                ...
                {
                    serviceNodes[description.S[j]] = new ServiceNode(j, random, new ExponentialVariable(random, description.mu[j]),
                        new QueueBuffer(), description.kappa[j], Nodes, Info, description.Theta.RoutingMatrixForNode(description.S[i]));
                    (Nodes[description.B[i]] as BalancerNode).AddServiceNode(serviceNodes[description.S[j]]);
                }
                //Дивайдеры
